...
// 배열.lenght 속성을 통해서 배열의 크기를 알 수 있다
let 배열크기 = 배열3.length;
...
2. 화살표 함수의 코드에서 마지막에 return 이 있으므로 return문이 5번 실행 될 것이다.
3. 또다른배열.push( return 된 결과 ) 의 코드가 5번 실행된다.

이 결과는 배열3에 담긴 모든 요소 끝에 문자열 A 부착되고
...
// map은 내부의 함수에 요소와 배열의 index 값을 매개변수로 전달 할 수 있고
...
// 원 배열에서 특정한 조건에 맞는 요소만 추출하여 다른 배열로 만들고 싶을때
// 내부 코드에서 특정 요소가 연살될때
