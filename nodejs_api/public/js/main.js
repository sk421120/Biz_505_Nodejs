...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (api_json_call) {
    ...
    api_json_call.addEventListener("click", async (e) => {
      // promise 방식으로 비동기 함수를 동기실으로 실행시키기
      ...
      let json_html = `name : <span>${result.j_name}</span>&nbsp;`;
      json_html += `addr : <span>${result.j_addr}</span>&nbsp;`;
      json_html += `tel : <span>${result.j_tel}</span>&nbsp;`;
      json_html += `age : <span>${result.j_age}</span>`;

      api_json_res.innerHTML = json_html;
      // .then((res) => res.json())
      // .then((result) => {
      //   console.log(result);
      // api_json_res.innerText = result;
      // });
    });
  }
});
